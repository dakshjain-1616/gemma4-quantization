...
def quantize_1bit(weight: torch.Tensor) -> torch.Tensor:
    """BitNet W1.58A8 — ternary {-1, 0, +1} scaled by mean absolute value."""
...
def measure_inference(model, tokenizer, texts=None, runs=3) -> Dict:
    global EVAL_TEXTS
    if texts is None:
        if EVAL_TEXTS is None:
            EVAL_TEXTS = load_wikitext2(50)
...
def main():
    ...
    model_fp16 = AutoModelForCausalLM.from_pretrained(
        MODEL_NAME, torch_dtype=torch.float16, device_map="cuda", token=HF_TOKEN
    )
    model_fp16.eval()
    logger.info(f"Model loaded. Params: {sum(p.numel() for p in model_fp16.parameters())/1e9:.2f}B")

    # ── Sensitivity analysis (on FP16 model) ───────────────────────────────
    logger.info("Running layer sensitivity analysis...")
    sensitivity = layer_sensitivity(model_fp16)
    ...
    for level, fn, m in quant_configs:
        ...
        if m is None:
            m = apply_quantization(model_fp16, fn)
        ...
        logger.info(f"  {level}: perplexity={ppl:.2f}, mem={mem_gb:.2f}GB, "
                    f"ms={speed['inference_time_ms']:.1f}")
        if m is not model_fp16:
